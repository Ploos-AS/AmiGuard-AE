...
TARGET := AmiGuardAE
HOST_SOURCES := src/main.c src/core.c src/arexx_dispatch.c src/arexx_amiga.c src/scanner_bridge.c src/result_store.c src/checksum.c
...
$(TARGET): $(HOST_SOURCES) include/amiguard_ae.h src/arexx_dispatch.h src/arexx_amiga.h src/scanner_bridge.h src/result_store.h src/checksum.h
	$(HOST_CC) $(HOST_CFLAGS) $(HOST_SOURCES) -o $@

amiga:
	$(AMIGA_CC) $(AMIGA_CFLAGS) $(AMIGA_SOURCES) -o $(TARGET).amiga

$(TEST_TARGET): tests/test_dispatch.c src/core.c src/arexx_dispatch.c src/scanner_bridge.c src/result_store.c src/checksum.c include/amiguard_ae.h src/arexx_dispatch.h src/scanner_bridge.h src/result_store.h src/checksum.h
	@mkdir -p build
	$(HOST_CC) $(HOST_CFLAGS) tests/test_dispatch.c src/core.c src/arexx_dispatch.c src/scanner_bridge.c src/result_store.c src/checksum.c -o $(TEST_TARGET)
...
check: test
	...
	@echo "M2.4 host qualification: PASS"
