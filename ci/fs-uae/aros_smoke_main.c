...
int main(void)
{
    ...
    ok = expect("STATUS", AMIGUARD_AE_RC_OK, "READY M3.3 scanner=connected") && ok;
    ...
    ok = expect("SIGNATURE.UPDATE now", AMIGUARD_AE_RC_ERROR,
                "ERROR SIGNATURE.UPDATE takes no arguments") && ok;
    ok = expect("SIGNATURE.UPDATE", AMIGUARD_AE_RC_ERROR,
                "ERROR signature updater unavailable") && ok;
    ...
    puts("M3.3 AROS signature update foundation smoke: PASS");
