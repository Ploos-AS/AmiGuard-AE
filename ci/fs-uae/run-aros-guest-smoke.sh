...
printf 'REXXMAST=%s\nREXXSYSLIB=%s\nNOTE=AROS qualifies native 68k M4.4 quarantine/restore dispatcher/core; production ARexx transport remains deferred local classic AmigaOS\n' "${rexxmast_host:-MISSING}" "${rexxlib_host:-MISSING}" > "$OUT_DIR/arexx-capabilities.txt"
cp "$NATIVE" "$aros_root/AmiGuardAE"; cp "$startup" "$startup.amiguard-ae-original"
cat > "$startup" <<'EOF'
FailAt 21
SYS:C/Echo "M4_4_GUEST_STARTED=1" >SYS:amiguard-ae-m4-4-started.txt
SYS:AmiGuardAE >SYS:amiguard-ae-m4-4-output.txt
SYS:C/Echo $RC >SYS:amiguard-ae-m4-4-rc.txt
SYS:C/Echo "M4_4_AFTER_AMIGUARD_AE=1" >SYS:amiguard-ae-m4-4-after.txt
EOF
rm -f "$aros_root"/amiguard-ae-m4-4-{started,output,rc,after}.txt
