...
static void trim_upper_token(const char *command, char *token, unsigned long size)
{
    ...
    while (*command != '\0' && !isspace((unsigned char)*command) && i + 1 < size) token[i++] = (char)toupper((unsigned char)*command++);
    token[i] = '\0';
}
...
{
    out->rc = rc; if (text == NULL) text = "";
    strncpy(out->result, text, AMIGUARD_AE_RESULT_MAX - 1); out->result[AMIGUARD_AE_RESULT_MAX - 1] = '\0';
}
static void dispatch_scanfile(const char *command, AmiGuardAERexxResult *out)
{
    const char *path = command_argument(command); AmiGuardAEScanResult scan; char text[AMIGUARD_AE_RESULT_MAX];
