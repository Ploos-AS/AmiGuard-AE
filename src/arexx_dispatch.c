...
static void audit_dispatch_event(const char *verb, const char *command,
                                 const AmiGuardAERexxResult *out)
{
    const char *event = 0;
    const char *status;
    const char *subject;
    char error[AMIGUARD_AE_AUDIT_DETAIL_MAX];
    if (!amiguard_ae_audit_available()) return;
    if (strcmp(verb, "SCAN") == 0 || strcmp(verb, "SCANFILE") == 0) event = "SCAN";
    else if (strcmp(verb, "QUARANTINE") == 0) event = "QUARANTINE";
    else if (strcmp(verb, "RESTORE") == 0) event = "RESTORE";
    else if (strcmp(verb, "SIGNATURE.UPDATE") == 0) event = "SIGNATURE.UPDATE";
    if (event == 0) return;
    status = out->rc == AMIGUARD_AE_RC_OK ? "OK" :
             (out->rc == AMIGUARD_AE_RC_WARN ? "WARN" : "ERROR");
    subject = command_argument(command);
    (void)amiguard_ae_audit_append(event, status, subject, out->result,
                                   error, sizeof(error));
}
...
static void dispatch_signature_status(const char *command, AmiGuardAERexxResult *out)
{
    ...
    if (!amiguard_ae_signature_available()) { set_result(out, AMIGUARD_AE_RC_ERROR, "ERROR signature backend unavailable"); return; }
    sprintf(text, "READY COUNT=%lu UPDATE=%s VERIFY=CRC32 AUTH=%s", amiguard_ae_signature_count(), amiguard_ae_signature_update_available() ? "AVAILABLE" : "UNAVAILABLE", amiguard_ae_signature_auth_available() ? "AVAILABLE" : "UNAVAILABLE");
...
{
    unsigned int i; if (text == 0 || strlen(text) != 8U) return 0;
    for (i = 0U; i < 8U; ++i) if (!isxdigit((unsigned char)text[i])) return 0;
    return 1;
}
...
{
    const char *cursor = command_argument(command); char path[256]; char expected[9]; char manifest[256]; AmiGuardAEChecksumResult checksum; char detail[AMIGUARD_AE_SIGNATURE_UPDATE_DETAIL_MAX]; char text[AMIGUARD_AE_RESULT_MAX]; unsigned int i;
    if (!next_token(&cursor, path, sizeof(path))) { set_result(out, AMIGUARD_AE_RC_ERROR, "ERROR SIGNATURE.UPDATE requires path CRC32 manifest"); return; }
    if (!next_token(&cursor, expected, sizeof(expected))) { set_result(out, AMIGUARD_AE_RC_ERROR, "ERROR SIGNATURE.UPDATE requires CRC32 and manifest"); return; }
    if (!next_token(&cursor, manifest, sizeof(manifest))) { set_result(out, AMIGUARD_AE_RC_ERROR, "ERROR SIGNATURE.UPDATE requires manifest"); return; }
    while (*cursor != '\0' && isspace((unsigned char)*cursor)) ++cursor;
    if (*cursor != '\0' || !valid_crc32_text(expected)) { set_result(out, AMIGUARD_AE_RC_ERROR, "ERROR invalid SIGNATURE.UPDATE arguments"); return; }
    for (i = 0U; i < 8U; ++i) expected[i] = (char)toupper((unsigned char)expected[i]);
    if (!amiguard_ae_signature_update_available()) { set_result(out, AMIGUARD_AE_RC_ERROR, "ERROR signature updater unavailable"); return; }
    if (!amiguard_ae_checksum_crc32(path, &checksum)) { sprintf(text, "ERROR %s", checksum.detail); set_result(out, AMIGUARD_AE_RC_ERROR, text); return; }
    if (strcmp(checksum.checksum, expected) != 0) { sprintf(text, "ERROR checksum mismatch expected=%s actual=%s", expected, checksum.checksum); set_result(out, AMIGUARD_AE_RC_ERROR, text); return; }
    if (!amiguard_ae_signature_auth_available()) { set_result(out, AMIGUARD_AE_RC_ERROR, "ERROR signature authenticator unavailable"); return; }
    if (!amiguard_ae_signature_authenticate(manifest, path, expected, detail, sizeof(detail))) { if (detail[0] == '\0') strcpy(detail, "authentication failed"); sprintf(text, "ERROR %s", detail); set_result(out, AMIGUARD_AE_RC_ERROR, text); return; }
    if (!amiguard_ae_signature_update(path, detail, sizeof(detail))) { if (detail[0] == '\0') strcpy(detail, "update failed"); sprintf(text, "ERROR %s", detail); set_result(out, AMIGUARD_AE_RC_ERROR, text); return; }
    amiguard_ae_signature_auth_commit(); if (detail[0] == '\0') strcpy(detail, "updated");
    sprintf(text, "UPDATED VERIFIED=CRC32 AUTH=%s %s", amiguard_ae_signature_auth_method(), detail); set_result(out, AMIGUARD_AE_RC_OK, text);
}

static void dispatch_quarantine(const char *command, AmiGuardAERexxResult *out)
{
    const char *path = command_argument(command); AmiGuardAEQuarantinePlan plan; char detail[AMIGUARD_AE_QUARANTINE_DETAIL_MAX]; char stored[AMIGUARD_AE_QUARANTINE_PATH_MAX]; char text[AMIGUARD_AE_RESULT_MAX];
    if (*path == '\0') { set_result(out, AMIGUARD_AE_RC_ERROR, "ERROR QUARANTINE requires path"); return; }
    if (!amiguard_ae_quarantine_available()) { set_result(out, AMIGUARD_AE_RC_ERROR, "ERROR quarantine directory unavailable"); return; }
    if (!amiguard_ae_quarantine_plan(path, &plan, detail, sizeof(detail))) { sprintf(text, "ERROR %s", detail); set_result(out, AMIGUARD_AE_RC_ERROR, text); return; }
    if (!amiguard_ae_quarantine_store(&plan, amiguard_ae_quarantine_directory(), stored, sizeof(stored), detail, sizeof(detail))) { sprintf(text, "ERROR %s", detail); set_result(out, AMIGUARD_AE_RC_ERROR, text); return; }
    if (strcmp(detail, "quarantine committed; source removal pending") == 0) { sprintf(text, "QUARANTINED ID=%s PATH=%s SOURCE=REMOVAL_PENDING", plan.id, stored); set_result(out, AMIGUARD_AE_RC_WARN, text); return; }
    sprintf(text, "QUARANTINED ID=%s PATH=%s", plan.id, stored); set_result(out, AMIGUARD_AE_RC_OK, text);
}

static void dispatch_restore(const char *command, AmiGuardAERexxResult *out)
{
    const char *id = command_argument(command); char restored[AMIGUARD_AE_QUARANTINE_PATH_MAX]; char detail[AMIGUARD_AE_QUARANTINE_DETAIL_MAX]; char text[AMIGUARD_AE_RESULT_MAX]; const char *p;
    if (*id == '\0') { set_result(out, AMIGUARD_AE_RC_ERROR, "ERROR RESTORE requires id"); return; }
    for (p = id; *p != '\0'; ++p) if (isspace((unsigned char)*p)) { set_result(out, AMIGUARD_AE_RC_ERROR, "ERROR RESTORE requires one id"); return; }
    if (!amiguard_ae_quarantine_restore(id, restored, sizeof(restored), detail, sizeof(detail))) { sprintf(text, "ERROR %s", detail); set_result(out, AMIGUARD_AE_RC_ERROR, text); return; }
    sprintf(text, "RESTORED ID=%s PATH=%s QUARANTINE=RETAINED", id, restored); set_result(out, AMIGUARD_AE_RC_OK, text);
}
...
void amiguard_ae_arexx_dispatch(const char *command, AmiGuardAERexxResult *out)
{
    ...
    else if (strcmp(verb, "HELP") == 0) set_result(out, AMIGUARD_AE_RC_OK, "PING VERSION STATUS HELP SCAN SCANFILE CHECKSUM IDENTIFY QUARANTINE RESTORE SIGNATURE.COUNT SIGNATURE.STATUS SIGNATURE.INFO SIGNATURE.UPDATE RESULT.STATUS RESULT.PATH RESULT.DETAIL RESULT.CLEAR");
    ...
    else if (strcmp(verb, "QUARANTINE") == 0) dispatch_quarantine(command, out);
    else if (strcmp(verb, "RESTORE") == 0) dispatch_restore(command, out);
    ...
    audit_dispatch_event(verb, command, out);
}
