...
{
    unsigned int i;
    if (text == 0 || strlen(text) != 8U) return 0;
...
{
    const char *arg = command_argument(command); const char *p; unsigned long n; char path[256]; char expected[9]; AmiGuardAEChecksumResult checksum; char detail[AMIGUARD_AE_SIGNATURE_UPDATE_DETAIL_MAX]; char text[AMIGUARD_AE_RESULT_MAX]; unsigned int i;
    if (*arg == '\0') { set_result(out, AMIGUARD_AE_RC_ERROR, "ERROR SIGNATURE.UPDATE requires path and CRC32"); return; }
    p = arg; while (*p != '\0' && !isspace((unsigned char)*p)) ++p; n = (unsigned long)(p - arg);
    if (n == 0UL || n >= sizeof(path)) { set_result(out, AMIGUARD_AE_RC_ERROR, "ERROR invalid signature database path"); return; }
    memcpy(path, arg, (size_t)n); path[n] = '\0'; while (*p != '\0' && isspace((unsigned char)*p)) ++p;
    if (*p == '\0') { set_result(out, AMIGUARD_AE_RC_ERROR, "ERROR SIGNATURE.UPDATE requires CRC32"); return; }
    arg = p; while (*p != '\0' && !isspace((unsigned char)*p)) ++p; n = (unsigned long)(p - arg);
    if (n != 8UL) { set_result(out, AMIGUARD_AE_RC_ERROR, "ERROR invalid CRC32"); return; }
    memcpy(expected, arg, 8U); expected[8] = '\0'; while (*p != '\0' && isspace((unsigned char)*p)) ++p;
    if (*p != '\0' || !valid_crc32_text(expected)) { set_result(out, AMIGUARD_AE_RC_ERROR, "ERROR invalid CRC32"); return; }
    ...
    if (detail[0] == '\0') strcpy(detail, "updated"); sprintf(text, "UPDATED VERIFIED=CRC32 %s", detail); set_result(out, AMIGUARD_AE_RC_OK, text);
}
...
void amiguard_ae_arexx_dispatch(const char *command, AmiGuardAERexxResult *out)
{
    ...
    else if (strcmp(verb, "STATUS") == 0) set_result(out, AMIGUARD_AE_RC_OK, amiguard_ae_scanner_available() ? "READY M3.6 scanner=connected" : "READY M3.6 scanner=not-connected");
