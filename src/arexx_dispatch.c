...
static void dispatch_signature_update(const char *command, AmiGuardAERexxResult *out)
{
    ...
    sprintf(text, "UPDATED VERIFIED=CRC32 AUTH=%s %s", amiguard_ae_signature_auth_method(), detail);
    set_result(out, AMIGUARD_AE_RC_OK, text);
}
...
static void dispatch_quarantine(const char *command, AmiGuardAERexxResult *out)
{
    ...
    char text[AMIGUARD_AE_RESULT_MAX];
    if (*path == '\0') {
        set_result(out, AMIGUARD_AE_RC_ERROR, "ERROR QUARANTINE requires path");
        return;
    }
    if (!amiguard_ae_quarantine_available()) {
        set_result(out, AMIGUARD_AE_RC_ERROR, "ERROR quarantine directory unavailable");
        return;
    }
    if (!amiguard_ae_quarantine_plan(path, &plan, detail, sizeof(detail))) {
        sprintf(text, "ERROR %s", detail);
        set_result(out, AMIGUARD_AE_RC_ERROR, text);
        return;
    }
    if (!amiguard_ae_quarantine_store(&plan, amiguard_ae_quarantine_directory(), stored, sizeof(stored), detail, sizeof(detail))) {
        sprintf(text, "ERROR %s", detail);
        set_result(out, AMIGUARD_AE_RC_ERROR, text);
        return;
    }
    if (strcmp(detail, "quarantine committed; source removal pending") == 0) {
        sprintf(text, "QUARANTINED ID=%s PATH=%s SOURCE=REMOVAL_PENDING", plan.id, stored);
        set_result(out, AMIGUARD_AE_RC_WARN, text);
        return;
    }
    sprintf(text, "QUARANTINED ID=%s PATH=%s", plan.id, stored);
    set_result(out, AMIGUARD_AE_RC_OK, text);
}

static void dispatch_restore(const char *command, AmiGuardAERexxResult *out)
{
    const char *id = command_argument(command);
    char restored[AMIGUARD_AE_QUARANTINE_PATH_MAX];
    char detail[AMIGUARD_AE_QUARANTINE_DETAIL_MAX];
    char text[AMIGUARD_AE_RESULT_MAX];
    const char *p;
    if (*id == '\0') {
        set_result(out, AMIGUARD_AE_RC_ERROR, "ERROR RESTORE requires id");
        return;
    }
    for (p = id; *p != '\0'; ++p) {
        if (isspace((unsigned char)*p)) {
            set_result(out, AMIGUARD_AE_RC_ERROR, "ERROR RESTORE requires one id");
            return;
        }
    }
    if (!amiguard_ae_quarantine_restore(id, restored, sizeof(restored), detail, sizeof(detail))) {
        sprintf(text, "ERROR %s", detail);
        set_result(out, AMIGUARD_AE_RC_ERROR, text);
        return;
    }
    sprintf(text, "RESTORED ID=%s PATH=%s QUARANTINE=RETAINED", id, restored);
    set_result(out, AMIGUARD_AE_RC_OK, text);
}
...
void amiguard_ae_arexx_dispatch(const char *command, AmiGuardAERexxResult *out)
{
    ...
    if (strcmp(verb, "PING") == 0)
        ...
        set_result(out, AMIGUARD_AE_RC_OK, amiguard_ae_scanner_available() ? "READY M4.4 scanner=connected" : "READY M4.4 scanner=not-connected");
    else if (strcmp(verb, "HELP") == 0)
        set_result(out, AMIGUARD_AE_RC_OK, "PING VERSION STATUS HELP SCAN SCANFILE CHECKSUM IDENTIFY QUARANTINE RESTORE SIGNATURE.COUNT SIGNATURE.STATUS SIGNATURE.INFO SIGNATURE.UPDATE RESULT.STATUS RESULT.PATH RESULT.DETAIL RESULT.CLEAR");
    ...
    else if (strcmp(verb, "QUARANTINE") == 0)
        dispatch_quarantine(command, out);
    else if (strcmp(verb, "RESTORE") == 0)
        dispatch_restore(command, out);
