...
static void dispatch_signature_info(const char *command, AmiGuardAERexxResult *out)
{
    ...
    sprintf(text, "INDEX=%lu TYPE=%s OFFSET=%lu LENGTH=%lu TEST_ONLY=%d NAME=%s", index, info.type, info.offset, info.length, info.test_only, info.name); set_result(out, AMIGUARD_AE_RC_OK, text);
}
