...
static int read_metadata(const char *path, AmiGuardAEQuarantinePlan *plan)
{
    ...
    while (fgets(line, sizeof(line), fp) != 0) {
        ...
        if (strncmp(line, "ID=", 3) == 0) {
            if (got_id || strlen(line + 3) >= sizeof(plan->id)) { fclose(fp); return 0; }
            ...
            if (got_crc || strlen(line + 6) != 8U) { fclose(fp); return 0; }
            strcpy(crc, line + 6);
            if (sscanf(crc, "%lx", &parsed_crc) != 1) { fclose(fp); return 0; }
            plan->source_crc32 = parsed_crc; got_crc = 1;
        } else if (strncmp(line, "SIZE=", 5) == 0) {
            if (got_size || sscanf(line + 5, "%lu", &parsed_size) != 1) { fclose(fp); return 0; }
                                 ...
                                 unsigned long detail_size)
{
    ...
    if (!amiguard_ae_checksum_crc32(stage, &sum) || sscanf(sum.checksum, "%lx", &actual_crc) != 1 || actual_crc != plan->source_crc32) {
                                   ...
                                   unsigned long detail_size)
{
    ...
    if (id == 0 || id[0] == '\0' || strlen(id) >= AMIGUARD_AE_QUARANTINE_ID_MAX || id[0] != 'Q') { detail_set(detail, detail_size, "invalid quarantine id"); return 0; }
    ...
    if (!amiguard_ae_checksum_crc32(object, &sum) || sscanf(sum.checksum, "%lx", &actual_crc) != 1 || actual_crc != plan.source_crc32) { detail_set(detail, detail_size, "quarantine object verification failed"); return 0; }
    fp = fopen(object, "rb");
    ...
    if (!amiguard_ae_checksum_crc32(stage, &sum) || sscanf(sum.checksum, "%lx", &actual_crc) != 1 || actual_crc != plan.source_crc32) { remove(stage); detail_set(detail, detail_size, "restored staging verification failed"); return 0; }
