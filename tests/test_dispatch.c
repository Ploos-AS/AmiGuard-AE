...
{
    if (path == 0 || strcmp(path, "build/fixture.sigdb") != 0) return 0;
...
int main(void)
{
    ...
    int failed = 0;
    if (!write_file("build/checksum-fixture.bin", checksum_data, 9UL) || !write_file("build/fixture.sigdb", checksum_data, 9UL) || !write_file("build/hunk-fixture.bin", hunk_data, 4UL) || !write_file("build/iff-fixture.bin", iff_data, 4UL)) return 1;
    failed += expect("PING",0,"PONG");
    failed += expect(" version ",0,"AmiGuard AE 0.3.0-m3.6");
    failed += expect("STATUS",0,"READY M3.6 scanner=not-connected");
    ...
    failed += expect("SIGNATURE.UPDATE",10,"ERROR SIGNATURE.UPDATE requires path and CRC32");
    failed += expect("SIGNATURE.UPDATE build/fixture.sigdb",10,"ERROR SIGNATURE.UPDATE requires CRC32");
    failed += expect("SIGNATURE.UPDATE build/fixture.sigdb ZZZZZZZZ",10,"ERROR invalid CRC32");
    failed += expect("SIGNATURE.UPDATE build/fixture.sigdb CBF43926",10,"ERROR signature updater unavailable");
    amiguard_ae_signature_set_count_provider(fake_signature_count); amiguard_ae_signature_set_info_provider(fake_signature_info);
    failed += expect("SIGNATURE.STATUS",0,"READY COUNT=2 UPDATE=UNAVAILABLE VERIFY=CRC32");
    ...
    failed += expect("SIGNATURE.STATUS",0,"READY COUNT=2 UPDATE=AVAILABLE VERIFY=CRC32");
    failed += expect("SIGNATURE.UPDATE build/fixture.sigdb 00000000",10,"ERROR checksum mismatch expected=00000000 actual=CBF43926");
    failed += expect("SIGNATURE.UPDATE build/fixture.sigdb cbf43926",0,"UPDATED VERIFIED=CRC32 fixture-v1");
    ...
    failed += expect("RESULT.STATUS",10,"ERROR no scan result"); failed += expect("SCAN",10,"ERROR SCAN requires path");
    amiguard_ae_scanner_set_provider(fake_scan); failed += expect("STATUS",0,"READY M3.6 scanner=connected");
    failed += expect("SCAN clean.bin",0,"CLEAN known-clean"); failed += expect("RESULT.STATUS",0,"CLEAN"); failed += expect("SCAN virus.bin",5,"INFECTED Test.Virus"); failed += expect("SCAN sample.bin",5,"SUSPICIOUS needs-analysis"); failed += expect("RESULT.CLEAR",0,"OK"); failed += expect("BOGUS",10,"ERROR unknown command"); failed += expect("",10,"ERROR empty command");
    remove("build/checksum-fixture.bin"); remove("build/fixture.sigdb"); remove("build/hunk-fixture.bin"); remove("build/iff-fixture.bin");
    if (failed != 0) return 1; puts("M3.6 verified signature activation qualification: PASS"); return 0;
}
